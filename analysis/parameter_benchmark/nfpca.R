...
colnames(data_performance) = c("nfpca", "value", "metric")
data_performance[, 3] = rep(c("accuracy", "sensitivity", "specificity", "F1"), num_fpca)

data_performance_JSD = data.frame(matrix(0, nrow = (3 * num_repeat * num_fpca), ncol =
                                           3))
colnames(data_performance_JSD) = c("nfpca", "value", "metric")
data_performance_JSD[, 3] = rep(c("0.25", "0.50", "0.75"), num_fpca * num_repeat)

for (i in 1:num_fpca) {
  nfpca = 6+i * 5
  data_performance[(1 + (i - 1) * 4):(4 + (i - 1) * 4), 1] = rep(nfpca, 4)
  data_performance_JSD[(1 + (i - 1) * 3 * num_repeat):(3 * num_repeat +
                                                         (i - 1) * 3 * num_repeat), 1] = rep(nfpca, 3 * num_repeat)
...
nfpca_list=6+seq(1,10)*5
colourCount=10
#plot
performance_plt <- data_performance %>%
  dplyr::mutate(
    nfpca = factor(x = nfpca,
                  levels = nfpca_list)
  ) %>%
  ggplot() +
  geom_point(aes(x = nfpca,
                 y = value,
                 color = nfpca),
             size = 5,
             alpha = 0.9) +
  ylim(0.25,1)+
  facet_wrap(. ~ metric) +
  labs(title = "Deconvolution parameter benchmarking(nfpca)",
       x = "nfpca",
       y = "Metric Value") +
  theme_classic()  +
...
performance_plt
colnames(data_performance_JSD)=c("nfpca","value","quantile")
