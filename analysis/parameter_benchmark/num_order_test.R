...
colnames(data_performance) = c("norder", "value", "metric")
data_performance[, 3] = rep(c("accuracy", "sensitivity", "specificity", "F1"), num_order)

data_performance_JSD = data.frame(matrix(0, nrow = (3 * num_repeat * num_order), ncol =
                                           3))
colnames(data_performance_JSD) = c("norder", "value", "quantile")
data_performance_JSD[, 3] = rep(c("0.25", "0.50", "0.75"), num_order * num_repeat)

#deconvolution spots
for (i in 1:num_order) {
  norder = i 
  data_performance[(1 + (i - 1) * 4):(4 + (i - 1) * 4), 1] = rep(norder, 4)
  data_performance_JSD[(1 + (i - 1) * 3 * num_repeat):(3 * num_repeat +
                                                         (i - 1) * 3 * num_repeat), 1] = rep(norder, 3 * num_repeat)
...
norder_list=seq(1,num_order)
colourCount=num_order
#plot
performance_plt <- data_performance %>%
  dplyr::mutate(
    norder = factor(x = norder,
                    levels = norder_list)
  ) %>%
  ggplot() +
  geom_point(aes(x = norder,
                 y = value,
                 color = norder),
             size = 5,
             alpha = 0.9) +
  ylim(0.25,1)+
  facet_wrap(. ~ metric) +
  labs(title = "Deconvolution parameter benchmarking(norder)",
       x = "norder",
       y = "Metric Value") +
  theme_classic()  +
  scale_fill_manual(values = colorRampPalette(brewer.pal(num_order, "Set3"))(colourCount))+
  theme(
