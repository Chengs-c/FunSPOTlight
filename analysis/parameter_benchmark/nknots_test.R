...
colnames(data_performance) = c("nknots", "value", "metric")
data_performance[, 3] = rep(c("accuracy", "sensitivity", "specificity", "F1"), num_knots)

data_performance_JSD = data.frame(matrix(0, nrow = (3 * num_repeat * num_knots), ncol =
                                           3))
colnames(data_performance_JSD) = c("nknots", "value", "quantile")
data_performance_JSD[, 3] = rep(c("0.25", "0.50", "0.75"), num_knots * num_repeat)

#deconvolution spots
for (i in 1:num_knots) {
  nknots = i * 20
  data_performance[(1 + (i - 1) * 4):(4 + (i - 1) * 4), 1] = rep(nknots, 4)
  data_performance_JSD[(1 + (i - 1) * 3 * num_repeat):(3 * num_repeat +
                                                         (i - 1) * 3 * num_repeat), 1] = rep(nknots, 3 * num_repeat)
...
nknots_list=seq(1,20)*20
colourCount=20
#plot
performance_plt <- data_performance %>%
  dplyr::mutate(
    nknots = factor(x = nknots,
                  levels = nknots_list)
  ) %>%
  ggplot() +
  geom_point(aes(x = nknots,
                 y = value,
                 color = nknots),
             size = 5,
             alpha = 0.9) +
  ylim(0.25,1)+
  facet_wrap(. ~ metric) +
  labs(title = "Deconvolution parameter benchmarking(knot_number)",
       x = "nknots",
       y = "Metric Value") +
  theme_classic()  +
