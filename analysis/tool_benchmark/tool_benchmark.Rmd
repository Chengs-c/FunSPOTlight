...
techs <- c("funspotlight","SPOTlight", "MuSiC weighted", "MuSiC all genes", "SCDC",
           "DeconRNAseq", "RCTD","SPOTlight-CoGAPS")
...
# MuSiC
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
music_deconv <- readRDS(file = sprintf("%s/%s/music_deconv.RDS",
                                                  an_tools, robj_dir))
...
spotlight_deconv <- readRDS(
  file = sprintf("%s/%s/spotlight_deconv.RDS",
                            an_tools, robj_dir))
...
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
scdc_obj <- readRDS(
  file = sprintf("%s/%s/scdc_deconv.RDS",
                            an_tools, robj_dir))
...
nmfreg_deconv <- readRDS(
  file = sprintf("%s/%s/nmfreg_ls_deconv.RDS",
                            an_tools, robj_dir))
...
deconrnaseq_deconv <- readRDS(
  file = sprintf("%s/%s/deconrnaseq_deconv.RDS",
                 an_tools, robj_dir))
...
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
myRCTD <- readRDS(
  file = sprintf("%s/%s/rctd_deconv.RDS",
                            an_tools, robj_dir))
...
colnames(rctd_deconv) <- gsub(pattern = "[[:punct:]]|[[:blank:]]", ".",
                              ...
                              perl = TRUE)
colnames(rctd_deconv)=colnames(synthetic_comp)
...
#   file = sprintf("%s/%s/CIBERSORTx_Results.txt",
#                             an_tools, robj_dir),
#   col_names = TRUE) %>%
...
#fun-spotlight
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
fun_spotlight_deconv=readRDS(file = sprintf("%s/%s/fun_spotlight_deconv.RDS",
                                  an_tools, robj_dir))
...
tech_ls <- list(
  ...
  "funspotlight"=fun_spotlight_deconv
  #"CIBERSORTx" = CIBERSORTx_deconv
)
# tech_ls <- list("MuSiC weighted" = music_mtrx_w,
#   "MuSiC all genes" = music_mtrx_a,
#   "SPOTlight" = spotlight_mtrx,
#   "RCTD" = rctd_deconv,
#   "SCDC" = scdc_deconv,
...
```{r}
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
source(glue::glue("{an_tools}/test_synthetic_performance_mod.R"))
...
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
cowplot::save_plot(
  filename = sprintf("%s/%s/class_benchmark1.pdf",
                     an_tools, plt_dir),
  plot = metric_plt,
  base_height = 6,
  base_width = 9)
...
JSD_benchmark <- JSD_df %>%
  ...
  scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Set3"))(colourCount))+
  theme(
    strip.text = element_text(hjust = 0.5, size = 14, face = "bold"),
    axis.text.x = element_text(size = 12, angle = 30, hjust = 1),
    axis.text.y = element_text(size = 12),
...
setwd("D:/study/undergraduate/spolight/code/SPOTlight_deconvolution_analysis-master")
cowplot::save_plot(
  filename = sprintf("%s/%s/JSD_benchmark1.pdf",
                     an_tools, plt_dir),
  plot = JSD_benchmark,
  base_height = 7,
  base_width = 11)
