...
spatial_decon_syn_assessment_fun_spotlight_fun=function(se_sc,
  ...
  setwd("D:\\study\\undergraduate\\spolight\\code\\fda_ST\\parameter_test\\result\\data\\multip_tech")
  saveRDS(marker_genes,file = sprintf("%s_markergene.rds",tech_list[i]))
  }else{
    
    setwd("D:\\study\\undergraduate\\spolight\\code\\fda_ST\\parameter_test\\result\\data\\multip_tech2")
    marker_genes=readRDS(sprintf("%s_markergene.rds",tech_list[i]))
